...
int main(int argc, char* argv[]) {
	...
	sqlite3_exec(db, "CREATE TABLE IF NOT EXISTS [Time.Isconna.Column] (\n\tdataset TEXT,\n\talg TEXT,\n\tf REAL,\n\tw REAL,\n\tg REAL,\n\tdecay REAL,\n\trow INTEGER,\n\tcol INTEGER,\n\texe_time REAL,\n\tCONSTRAINT [PrimaryKey_Time.Isconna.Column]\n\t\tPRIMARY KEY (dataset, alg, f, w, g, decay, row, col)\n);", nullptr, nullptr, nullptr);
	sqlite3_stmt* stmt; // language=sql
	sqlite3_prepare_v2(db, "SELECT f, w, g, decay, row FROM [AUROC.Isconna] WHERE alg=:alg AND dataset=:dataset ORDER BY roc_auc DESC LIMIT 1;", -1, &stmt, nullptr);
	...
	int shapeCMS[2] = {sqlite3_column_int(stmt, 4), 0};
	...
	// language=sql
	sqlite3_prepare_v2(db, "REPLACE INTO [Time.Isconna.Column] VALUES (:dataset, :alg, :f, :w, :g, :decay, :row, :col, :exe_time);", -1, &stmt, nullptr);
	const auto score = new double[n];
	const auto times = new time_t[numRepeat];
	for (shapeCMS[1] = 500; shapeCMS[1] <= 3000; shapeCMS[1] += 500) {
