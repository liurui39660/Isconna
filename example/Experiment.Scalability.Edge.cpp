...
int main(int argc, char* argv[]) {
	...
	sqlite3_exec(db, "CREATE TABLE IF NOT EXISTS [Time.Isconna.Edge] (\n\tdataset TEXT,\n\talg TEXT,\n\tf REAL,\n\tw REAL,\n\tg REAL,\n\tdecay REAL,\n\trow INTEGER,\n\tcol INTEGER,\n\tn INTEGER,\n\texe_time REAL,\n\tCONSTRAINT [PrimaryKey_Time.Isconna.Edge]\n\t\tPRIMARY KEY (dataset, alg, f, w, g, decay, row, col, n)\n);", nullptr, nullptr, nullptr);
	sqlite3_stmt* stmt; // language=sql
	sqlite3_prepare_v2(db, "SELECT f, w, g, decay, row, col FROM [AUROC.Isconna] WHERE alg=:alg AND dataset=:dataset ORDER BY roc_auc DESC LIMIT 1;", -1, &stmt, nullptr);
	...
	// language=sql
	sqlite3_prepare_v2(db, "REPLACE INTO [Time.Isconna.Edge] VALUES (:dataset, :alg, :f, :w, :g, :decay, :row, :col, :n, :exe_time);", -1, &stmt, nullptr);
	...
	for (const auto nn: ns) {
		...
		sqlite3_bind_int(stmt, sqlite3_bind_parameter_index(stmt, ":n"), nn);
